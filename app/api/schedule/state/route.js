...
export async function GET() {
  try {
    ...
    const [settingsRows, fixedEvents, tutorialSlots, taOfficeHours, tas, tutorialVenues, staff] = await Promise.all([
      ...
      sbSelect('tas', `${courseFilter}&select=id,name,office_hours_only,oh_cap_hours,email,duty_tag&order=name.asc`),
      sbSelect(
    ...
    return Response.json({
      ...
      tas: tas.map((t) => ({ id: t.id, name: t.name, officeHoursOnly: t.office_hours_only, ohCapHours: num(t.oh_cap_hours), email: t.email, dutyTag: t.duty_tag })),
