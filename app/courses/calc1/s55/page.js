...
function IncomeStreamWidget() {
  ...
  const [rate, setRate]   = useState(1200);   // f(t) PKR thousands/yr
  const [r, setR]         = useState(8);      // interest % compounded cont.
  const [T, setT]         = useState(2);      // term years
  const [view, setView]   = useState('fv');   // 'fv' | 'pv'
  ...
  const draw = () => {
    ...
    ctx.beginPath(); ctx.moveTo(pad.l - 10, tY); ctx.lineTo(W - pad.r + 10, tY); ctx.stroke();
    // Arrowhead
    ctx.beginPath(); ctx.moveTo(W - pad.r + 10, tY); ctx.lineTo(W - pad.r, tY - 6); ctx.lineTo(W - pad.r, tY + 6); ctx.closePath(); ctx.fillStyle = '#4b5563'; ctx.fill();

    // Draw n=6 subintervals showing growth bars
    const n = 6;
    const colors = ['#ef4444','#f59e0b','#22c55e','#3b82f6','#a78bfa','#38c9b0'];
    for (let j = 0; j < n; j++) {
      const tj = j * T / n;
      const xj = pad.l + tj / T * gW;
      const dt = T / n;
      const deposit = rate * dt; // money deposited this slice

      let barH;
      if (view === 'fv') {
        // future value: grows for (T - tj) years
        const fv = deposit * Math.exp(rDec * (T - tj));
        barH = Math.min(120, fv / (rate * T) * 100);
      } else {
        // present value: discounted by e^(-r*tj)
        const pv = deposit * Math.exp(-rDec * tj);
        barH = Math.min(120, pv / (rate * T) * 100);
      }

      // Bar
      ctx.fillStyle = colors[j] + '44';
      ctx.strokeStyle = colors[j];
      ctx.lineWidth = 1.5;
      ctx.fillRect(xj, tY - barH, gW / n - 2, barH);
      ctx.strokeRect(xj, tY - barH, gW / n - 2, barH);

      // Tick
      ctx.strokeStyle = '#6b7280'; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.moveTo(xj, tY - 4); ctx.lineTo(xj, tY + 4); ctx.stroke();
      ctx.fillStyle = '#9ca3af'; ctx.font = '10px IBM Plex Mono,monospace'; ctx.textAlign = 'center';
      ctx.fillText('t'+(j), xj, tY + 16);
    }

    // T tick
    const xT = pad.l + gW;
    ctx.strokeStyle = '#d4a017'; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(xT, tY - 8); ctx.lineTo(xT, tY + 8); ctx.stroke();
    ctx.fillStyle = '#d4a017'; ctx.font = 'bold 11px IBM Plex Mono,monospace'; ctx.textAlign = 'center';
    ctx.fillText('T='+T, xT, tY + 20);

    // 0 label
    ctx.fillStyle = '#9ca3af'; ctx.font = '10px IBM Plex Mono,monospace'; ctx.textAlign = 'center';
    ctx.fillText('0', pad.l, tY + 16);
    ctx.fillText('t (years)', W / 2, H - 4);

    // Arrow showing growth direction
    if (view === 'fv') {
      ctx.fillStyle = '#38c9b0'; ctx.font = 'bold 11px IBM Plex Mono,monospace'; ctx.textAlign = 'left';
      ctx.fillText('← each slice grows to T', pad.l, 28);
      ctx.fillStyle = '#6b7280'; ctx.font = '10px IBM Plex Mono,monospace';
      ctx.fillText('taller bar = deposited earlier = more growth', pad.l, 44);
    } else {
      ctx.fillStyle = '#a78bfa'; ctx.font = 'bold 11px IBM Plex Mono,monospace'; ctx.textAlign = 'left';
      ctx.fillText('discounted back to t=0 →', pad.l, 28);
      ctx.fillStyle = '#6b7280'; ctx.font = '10px IBM Plex Mono,monospace';
      ctx.fillText('taller bar = deposited sooner = less discounting', pad.l, 44);
    }
  };

  useEffect(() => { draw(); }, [rate, r, T, view]);
  useEffect(() => { const h = () => draw(); window.addEventListener('resize', h, { passive: true }); return () => window.removeEventListener('resize', h); }, [rate, r, T, view]);

  const rDec = r / 100;
  const FV = rate * Math.exp(rDec * T) * (1 - Math.exp(-rDec * T)) / rDec;
  const PV = rate * (1 - Math.exp(-rDec * T)) / rDec;

  const LS = { fontFamily:"'IBM Plex Mono',monospace", fontSize:'.68rem', letterSpacing:'.1em', textTransform:'uppercase', color:'#9ca3af' };
  const SS = { fontFamily:"'IBM Plex Mono',monospace", fontSize:'.78rem', background:'#1f2937', color:'#e8e2d9', border:'1px solid #374151', borderRadius:'6px', padding:'5px 9px', width:'80px', outline:'none' };

  return (
    <div style={S.widget}>
      <div style={S.wt}>💰 Income Stream — Future vs Present Value Visualiser</div>

      {/* Mode toggle */}
      <div style={{ display:'flex', gap:'8px', marginBottom:'14px' }}>
        {[['fv','Future Value (FV)'],['pv','Present Value (PV)']].map(([k,lbl])=>(
          <button key={k} onClick={()=>setView(k)} style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'.7rem', letterSpacing:'.08em', textTransform:'uppercase', background:view===k?'#d4a017':'#1f2937', color:view===k?'#111827':'#9ca3af', border:`1px solid ${view===k?'#d4a017':'#374151'}`, borderRadius:'5px', padding:'5px 14px', cursor:'pointer' }}>{lbl}</button>
        ))}
      </div>

      <canvas ref={canvasRef} style={{ display:'block', width:'100%', height:'220px', borderRadius:'8px', background:'#111827' }}/>

      {/* Controls */}
      <div style={{ display:'flex', flexWrap:'wrap', gap:'18px', marginTop:'16px' }}>
        <div><div style={{...LS,marginBottom:'4px'}}>Rate f(t) (PKR 000/yr)</div>
          <input type="number" value={rate} onChange={e=>setRate(+e.target.value||1200)} style={SS}/></div>
        <div><div style={{...LS,marginBottom:'4px'}}>Interest rate r (%)</div>
          <input type="number" value={r} min="1" max="30" step="0.5" onChange={e=>setR(+e.target.value||8)} style={SS}/></div>
        <div style={{ flex:1, minWidth:'140px' }}>
          <div style={{...LS,marginBottom:'4px'}}>Term T = <span style={{color:'#d4a017',fontWeight:700}}>{T} yr</span></div>
          <input type="range" min="1" max="10" step="0.5" value={T} onInput={e=>setT(+e.target.value)} style={{ WebkitAppearance:'none', width:'100%', height:'5px', background:'#374151', borderRadius:'3px', outline:'none' }}/>
        </div>
      </div>

      {/* Results */}
      <div style={{ display:'flex', gap:'10px', marginTop:'14px', flexWrap:'wrap' }}>
        <div style={{ background:'#1f2937', border:'1px solid #d4a017', borderRadius:'8px', padding:'10px 16px', flex:1, minWidth:'140px' }}>
          <div style={{...LS, color:'#d4a017', marginBottom:'4px'}}>Future Value</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'1.1rem', color:'#fbbf24', fontWeight:700 }}>PKR {(FV).toFixed(2)}</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'.62rem', color:'#6b7280', marginTop:'2px' }}>{'FV = e^(rT) ∫₀ᵀ f(t)e^(−rt) dt'}</div>
        </div>
        <div style={{ background:'#1f2937', border:'1px solid #a78bfa', borderRadius:'8px', padding:'10px 16px', flex:1, minWidth:'140px' }}>
          <div style={{...LS, color:'#a78bfa', marginBottom:'4px'}}>Present Value</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'1.1rem', color:'#c4b5fd', fontWeight:700 }}>PKR {PV.toFixed(2)}</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'.62rem', color:'#6b7280', marginTop:'2px' }}>{'PV = ∫₀ᵀ f(t)e^(−rt) dt'}</div>
        </div>
        <div style={{ background:'#1f2937', border:'1px solid #38c9b0', borderRadius:'8px', padding:'10px 16px', flex:1, minWidth:'140px' }}>
          <div style={{...LS, color:'#38c9b0', marginBottom:'4px'}}>Relationship</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'.82rem', color:'#6ee7b7', fontWeight:600 }}>{'FV = e^(rT) × PV'}</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'.78rem', color:'#6b7280', marginTop:'4px' }}>= e^({(rDec*T).toFixed(3)}) × {PV.toFixed(2)}</div>
        </div>
      </div>
    </div>
  );
...
function SurplusWidget() {
  const canvasRef = useRef(null);
  const [mode, setMode]  = useState('cs');   // 'cs' | 'ps'
  const [q0, setQ0]      = useState(3);
  const [dType, setDType] = useState('linear'); // demand curve type
  const [sType, setSType] = useState('linear'); // supply curve type
  ...
  const draw = () => {
    ...
    const yMax = Math.max(D(0), Sv ? Sv(0) : 0, p0) * 1.15;
    ...
    [1,2,3,4,5].forEach(x => {
      ...
      ctx.fillText(x, tX(x), pad.t + gH + 15);
    });
    [10,20,30,40,50,60,70,80].forEach(y => {
      if (y > yMax) return;
      ctx.strokeStyle = '#1f2937'; ctx.lineWidth = 0.8;
      ...
      ctx.fillText(y, pad.l - 4, tY(y) + 4);
    });

    if (mode === 'cs') {
      // CS = ∫₀^q0 D(q)dq − p0*q0
      // Shade: area under D curve from 0 to q0
      ...
      ctx.fillStyle = 'rgba(41,128,185,0.18)'; ctx.fill();
      // Shade CS (above p0, below D)
      ctx.beginPath(); ctx.moveTo(tX(0), tY(D(0)));
      for (let i = 0; i <= 200; i++) { const q = i * q0 / 200; ctx.lineTo(tX(q), tY(D(q))); }
      ctx.lineTo(tX(q0), tY(p0)); ctx.lineTo(tX(0), tY(p0)); ctx.closePath();
      ctx.fillStyle = 'rgba(56,201,176,0.35)'; ctx.fill();
      // Shade actual expenditure (rectangle p0 * q0)
      ctx.fillStyle = 'rgba(41,128,185,0.18)';
      ctx.fillRect(tX(0), tY(p0), tX(q0) - tX(0), tY(0) - tY(p0));
    } else {
      // PS = p0*q0 − ∫₀^q0 S(q)dq
      // Shade area under supply curve
      ...
      // Shade PS (above supply, below p0)
      ctx.beginPath(); ctx.moveTo(tX(0), tY(p0));
      ctx.lineTo(tX(q0), tY(p0));
      for (let i = 200; i >= 0; i--) { const q = i * q0 / 200; ctx.lineTo(tX(q), tY(Sv(q))); }
      ctx.closePath();
      ctx.fillStyle = 'rgba(212,160,23,0.40)'; ctx.fill();
    }

    // Demand curve
    ctx.beginPath();
    for (let i = 0; i <= 300; i++) { const q = i * xMax / 300; const y = D(q); if (i===0) ctx.moveTo(tX(q),tY(y)); else ctx.lineTo(tX(q),tY(y)); }
    ctx.strokeStyle = '#ef4444'; ctx.lineWidth = 2.5; ctx.stroke();
    ctx.fillStyle = '#ef4444'; ctx.font = 'bold 11px serif'; ctx.textAlign = 'left';
    ctx.fillText('D(q)', tX(xMax * 0.6), tY(D(xMax * 0.6)) - 8);
    ...
    if (mode === 'ps') {
      ...
      ctx.fillStyle = '#22c55e'; ctx.font = 'bold 11px serif';
      ctx.fillText('S(q)', tX(q0 * 0.6), tY(Sv(q0 * 0.6)) + 16);
    }
    ...
    // p0, q0 labels
    ctx.fillStyle = '#d4a017'; ctx.font = 'bold 11px IBM Plex Mono,monospace'; ctx.textAlign = 'right';
    ...
    ctx.beginPath(); ctx.moveTo(pad.l, pad.t); ctx.lineTo(pad.l, pad.t + gH + 5); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(pad.l - 5, pad.t + gH); ctx.lineTo(pad.l + gW + 5, pad.t + gH); ctx.stroke();
    ...
    // Surplus label in shaded region
    if (mode === 'cs') {
      ctx.fillStyle = '#38c9b0'; ctx.font = 'bold 11px IBM Plex Mono,monospace'; ctx.textAlign = 'center';
      ctx.fillText("CS", tX(q0 / 2), tY((D(q0 / 2) + p0) / 2));
    } else {
      ctx.fillStyle = '#d4a017'; ctx.font = 'bold 11px IBM Plex Mono,monospace'; ctx.textAlign = 'center';
      ctx.fillText("PS", tX(q0 / 2), tY((p0 + Sv(q0 / 2)) / 2));
    }
  };

  useEffect(() => { draw(); }, [mode, q0, dType, sType]);
  useEffect(() => { const h = () => draw(); window.addEventListener('resize', h, { passive: true }); return () => window.removeEventListener('resize', h); }, [mode, q0, dType, sType]);
  ...
  let wsD = 0, wsS = 0;
  for (let i = 0; i < N; i++) { const q = (i + 0.5) * dq; wsD += D(q) * dq; wsS += Sv(q) * dq; }
  const CS = wsD - p0cs * q0;
  const PS = p0ps * q0 - wsS;

  const LS = { fontFamily:"'IBM Plex Mono',monospace", fontSize:'.68rem', letterSpacing:'.1em', textTransform:'uppercase', color:'#9ca3af' };
  const SS2 = { fontFamily:"'IBM Plex Mono',monospace", fontSize:'.78rem', background:'#1f2937', color:'#e8e2d9', border:'1px solid #374151', borderRadius:'6px', padding:'5px 9px', cursor:'pointer' };

  return (
    <div style={S.widget}>
      <div style={S.wt}>📊 Consumer & Producer Surplus — Interactive</div>

      {/* Mode toggle */}
      <div style={{ display:'flex', gap:'8px', marginBottom:'14px', flexWrap:'wrap' }}>
        {[['cs',"Consumer's Surplus (CS)"],['ps',"Producer's Surplus (PS)"]].map(([k,lbl])=>(
          <button key={k} onClick={()=>setMode(k)} style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'.7rem', letterSpacing:'.08em', textTransform:'uppercase', background:k==='cs'?mode==='cs'?'#38c9b0':'#1f2937':mode==='ps'?'#d4a017':'#1f2937', color:k==='cs'?mode==='cs'?'#111827':'#9ca3af':mode==='ps'?'#111827':'#9ca3af', border:`1px solid ${k==='cs'?mode==='cs'?'#38c9b0':'#374151':mode==='ps'?'#d4a017':'#374151'}`, borderRadius:'5px', padding:'5px 14px', cursor:'pointer' }}>{lbl}</button>
        ))}
      </div>
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:'14px', marginTop:'14px', alignItems:'flex-end' }}>
        <div>
          <div style={{...LS,marginBottom:'4px'}}>Demand curve</div>
          <select value={dType} onChange={e=>setDType(e.target.value)} style={SS2}>
        ...
        {mode === 'ps' && (
          <div>
            <div style={{...LS,marginBottom:'4px'}}>Supply curve</div>
            <select value={sType} onChange={e=>setSType(e.target.value)} style={SS2}>
        ...
        <div style={{ flex:1, minWidth:'160px' }}>
          <div style={{...LS,marginBottom:'4px'}}>q₀ = <span style={{color:'#d4a017',fontWeight:700}}>{q0.toFixed(1)}</span> units</div>
          <input type="range" min="0.5" max="4.5" step="0.1" value={q0} onInput={e=>setQ0(+e.target.value)} style={{ WebkitAppearance:'none', width:'100%', height:'5px', background:'#374151', borderRadius:'3px', outline:'none' }}/>
        </div>
      </div>

      {/* Stat cards */}
      <div style={{ display:'flex', gap:'10px', marginTop:'14px', flexWrap:'wrap' }}>
        <div style={{ background:'#1f2937', border:'1px solid #38c9b0', borderRadius:'8px', padding:'10px 14px', flex:1, minWidth:'130px' }}>
          <div style={{...LS,color:'#38c9b0',marginBottom:'4px'}}>Consumer's Surplus</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'1.05rem', color:'#6ee7b7', fontWeight:700 }}>{CS.toFixed(2)}</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'.62rem', color:'#6b7280', marginTop:'2px' }}>∫D(q)dq − p₀q₀</div>
        </div>
        <div style={{ background:'#1f2937', border:'1px solid #d4a017', borderRadius:'8px', padding:'10px 14px', flex:1, minWidth:'130px' }}>
          <div style={{...LS,color:'#d4a017',marginBottom:'4px'}}>Producer's Surplus</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'1.05rem', color:'#fbbf24', fontWeight:700 }}>{PS.toFixed(2)}</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'.62rem', color:'#6b7280', marginTop:'2px' }}>p₀q₀ − ∫S(q)dq</div>
        </div>
        <div style={{ background:'#1f2937', border:'1px solid #6366f1', borderRadius:'8px', padding:'10px 14px', flex:1, minWidth:'130px' }}>
          <div style={{...LS,color:'#818cf8',marginBottom:'4px'}}>Market Price p₀</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'1.05rem', color:'#a5b4fc', fontWeight:700 }}>{(mode==='cs'?p0cs:p0ps).toFixed(2)}</div>
          <div style={{ fontFamily:"'IBM Plex Mono',monospace", fontSize:'.62rem', color:'#6b7280', marginTop:'2px' }}>at q₀ = {q0.toFixed(1)} units</div>
        </div>
      </div>
    </div>
  );
          ...
          <div style={S.lecInner} className="lec-inner-m">
            ...
            <section id="hook" className="lec-sec">
              ...
              <div style={{display:'flex',flexDirection:'column',gap:'12px',margin:'20px 0'}}>
                {[
                  {n:'Step 1',col:'#d4a017',txt:'Imagine slicing time into tiny pieces — say, each piece is Δt years long. During slice j, your business earns f(tⱼ)·Δt rupees.'},
                  {n:'Step 2',col:'#38c9b0',txt:'That money immediately goes into a bank account earning interest rate r (compounded continuously). It will sit there for (T − tⱼ) more years until the end of the term.'},
                  {n:'Step 3',col:'#ef4444',txt:'By the continuous compounding formula, that one slice grows to: f(tⱼ)·e^(r·(T−tⱼ))·Δt rupees by year T.'},
                  {n:'Step 4',col:'#a78bfa',txt:'Add up ALL the slices from t=0 to t=T and take the limit. The sum becomes an integral: FV = ∫₀ᵀ f(t)·e^(r(T−t)) dt.'},
                ].map((step,i)=>(
                  <div key={i} style={{display:'flex',gap:'14px',alignItems:'flex-start',background:'#fff',border:'1px solid #e0d6c8',borderRadius:'10px',padding:'14px 18px'}}>
                    <div style={{width:'70px',fontFamily:"'IBM Plex Mono',monospace",fontSize:'.7rem',fontWeight:700,color:step.col,flexShrink:0,paddingTop:'2px'}}>{step.n}</div>
                    <p style={{...S.p,margin:0}}>{step.txt}</p>
                  </div>
                ))}
              </div>
            </section>
              ...
              <div style={{...S.card,...S.cardAl}}>
                ...
                <div style={{background:'#fdf8f0',border:'1px solid #e0d6c8',borderRadius:'10px',padding:'18px',margin:'14px 0'}}>
                  ...
                  <svg viewBox="0 0 540 110" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',maxWidth:'600px',display:'block',margin:'0 auto'}}>
                    ...
                    <text x="470" y="22" fontSize="10" fontFamily="monospace" fill="#d4a017" textAnchor="middle">grows e^(0.08(2−tⱼ))</text>
                    <text x="470" y="34" fontSize="9" fontFamily="monospace" fill="#888" textAnchor="middle">× (deposit)</text>
                    <text x="270" y="106" fontSize="11" fontFamily="monospace" fill="#555" textAnchor="middle">years</text>
                  </svg>
                </div>
            ...
            <section id="ws" className="lec-sec">
              ...
              {/* Three-diagram visual */}
              <div style={{background:'#fff',border:'1px solid #e0d6c8',borderRadius:'12px',padding:'22px 24px',marginBottom:'24px'}}>
                <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:'.65rem',letterSpacing:'.18em',textTransform:'uppercase',color:'#d4a017',marginBottom:'14px'}}>Willingness to Spend = Area Under Demand Curve</div>
                <svg viewBox="0 0 400 180" xmlns="http://www.w3.org/2000/svg" style={{width:'100%',maxWidth:'480px',display:'block',margin:'0 auto'}}>
                  <rect width="400" height="180" fill="#fdf8f0" rx="8"/>
                  {/* Axes */}
                  <line x1="40" y1="150" x2="360" y2="150" stroke="#555" strokeWidth="1.5"/>
                  <line x1="40" y1="20" x2="40" y2="155" stroke="#555" strokeWidth="1.5"/>
                  <polygon points="360,150 353,147 353,153" fill="#555"/>
                  <polygon points="40,20 37,27 43,27" fill="#555"/>
                  {/* Shaded WS area */}
                  <path d="M40,30 Q140,55 240,110 L240,150 L40,150 Z" fill="rgba(41,128,185,0.22)"/>
                  {/* Demand curve */}
                  <path d="M40,30 Q140,55 240,110 Q310,145 360,158" fill="none" stroke="#ef4444" strokeWidth="2.5"/>
                  {/* p0 line */}
                  <line x1="40" y1="110" x2="240" y2="110" stroke="#d4a017" strokeWidth="1.5" strokeDasharray="5,3"/>
                  <line x1="240" y1="110" x2="240" y2="150" stroke="#d4a017" strokeWidth="1.5" strokeDasharray="5,3"/>
                  {/* Hatching label */}
                  <text x="110" y="95" fontSize="11" fontFamily="monospace" fill="#2980b9" fontWeight="700" textAnchor="middle">WS = ∫D(q)dq</text>
                  <text x="110" y="109" fontSize="10" fontFamily="monospace" fill="#6b7280" textAnchor="middle">(total area)</text>
                  {/* Labels */}
                  <text x="355" y="145" fontSize="10" fontFamily="monospace" fill="#555">q</text>
                  <text x="32" y="18" fontSize="10" fontFamily="monospace" fill="#555">p</text>
                  <text x="340" y="152" fontSize="10" fontFamily="serif" fill="#ef4444" fontStyle="italic">D(q)</text>
                  <text x="240" y="165" fontSize="10" fontFamily="monospace" fill="#d4a017" textAnchor="middle">q₀</text>
                  <text x="32" y="113" fontSize="10" fontFamily="monospace" fill="#d4a017" textAnchor="end">p₀</text>
                </svg>
              </div>
