...
// UPSERT — insert, or update in place on a conflicting unique constraint
// (`onConflict` is a comma-separated list of the constraint's columns).
export async function sbUpsert(table, row, onConflict) {
  ...
  if (!res.ok) throw new Error(`sbUpsert(${table}) failed: ${res.status} ${await res.text()}`);
  return asJson(res);
}
